# ==============================================================================
# MONKEY-PATCH : COMPATIBILITÉ HUGGINGFACE HUB POUR WHISPERX/PYANNOTE (TRANSFORMERS 5+)
# ==============================================================================
        ...
        # LOGIQUE DE TA BIBLIOTHÈQUE DE VOIX (F5-TTS, ChatterBox, OmniVoice)
            ...
            try:
                ...
                if cloning_engine == "F5-TTS":
                    ...
                    f5_engine = F5TTS()
                    f5_engine.generate(text=text, ref_audio=ref_audio, ref_text=ref_text, file_outfile=output_file)
                    generated_successfully = True
                elif cloning_engine == "ChatterBox Multilingual":
                    from chatterbox import ChatterBox
                    chatter_engine = ChatterBox()
                    chatter_engine.generate(text=text, ref_audio=ref_audio, ref_text=ref_text, lang=get_kokoro_lang_code(TRANSLATE_AUDIO_TO), output_file=output_file)
                    generated_successfully = True
...
class SoniTranslate(SoniTrCache):
    ...
    def batch_multilingual_media_conversion(self, *kwargs):
        ...
        # SÉCURITÉ : Si l'utilisateur laisse le lien vide (None), on le remplace par ""
        link_media_arg = kwargs[1] if kwargs[1] is not None else ""
        link_media_arg = [x.strip() for x in link_media_arg.split(',') if x.strip()]
        link_media_arg = get_link_list(link_media_arg)

        # SÉCURITÉ : Même chose pour le chemin de fichier local
        path_arg = kwargs[2] if kwargs[2] is not None else ""
        path_arg = [x.strip() for x in path_arg.split(',') if x.strip()]
