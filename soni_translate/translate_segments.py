...
import time
import os  # AJOUTÉ
from huggingface_hub import InferenceClient  # AJOUTÉ

TRANSLATION_PROCESS_OPTIONS = [
    "zephyr_7b_beta",  # AJOUTÉ
...
def zephyr_sequential(segments, target, source=None):
    """
    Traduction séquentielle utilisant Zephyr-7b-beta via Hugging Face.
    AJOUT PERSONNALISÉ.
    """
    translated_segments = copy.deepcopy(segments)
    
    # Récupération du token HF (variable d'env ou hardcodé si besoin)
    hf_token = os.getenv("HF_TOKEN")
    if not hf_token:
        logger.warning("HF_TOKEN non trouvé dans les variables d'environnement. Zephyr risque d'échouer.")

    # Configuration du client Hugging Face
    repo_id = "HuggingFaceH4/zephyr-7b-beta"
    client = InferenceClient(model=repo_id, token=hf_token)

    progress_bar = tqdm(total=len(segments), desc="Translating with Zephyr")

    # Gestion des langues pour le prompt
    lang_tg = re.sub(r'\([^)]*\)', '', INVERTED_LANGUAGES[target]).strip()
    lang_sc = ""
    if source:
        lang_sc = re.sub(r'\([^)]*\)', '', INVERTED_LANGUAGES[source]).strip()
    else:
        lang_sc = "the original language"
    ...
    for i, line in enumerate(translated_segments):
        ...
        prompt = f"""<|system|>
You are a professional translator. Translate the following text from {lang_sc} to {lang_tg}.
Return ONLY the translated text. Do not add explanations, quotes, or notes.
</s>
<|user|>
{text}
</s>
<|assistant|>"""
        ...
        try:
            ...
            logger.error(
                f"Zephyr Error: {str(error)} >> Segment {start} fallback to Google Translate"
            )
            # Fallback Google Translator en cas d'erreur
            translator = GoogleTranslator(
                source=fixed_source, target=fixed_target
            )
            translated_text = translator.translate(text.strip())
...
def translate_text(
    ...
    match translation_process:
        case "zephyr_7b_beta": # AJOUTÉ
            return zephyr_sequential(segments, target, source)
